...
async function getMovieShowTimes(movieId: number, startDate: string, endDate: string) {
  const sql = `
    SELECT 
    t.theater_id,
...
async function getDayShowTimes(startDate: string, endDate: string) {
  const sql = `
    SELECT 
    t.theater_id,
    t.name AS theater_name,
...
async function addShowTime(data: {
  ...
  start_time: string;
  end_time: string;
  seatTypes: Array<{ seatType: string; price: number; totalSeats: number }>;
}) {
  const { movie_id, screen_id, start_time, end_time, seatTypes } = data;
  ...
  // Auto-generate seats if screen has none
